...
Additional workstations, already onsite, were imaged from that validated image and held in onsite storage rather than finished and placed immediately. Completing the full sequence right after imaging — pre-domain-join preparation, domain join, and post-domain-join PowerShell-scripted build-out (with only a handful of manual steps left after that) — would have put an Active Directory computer account into the domain well before final placement and go-live. If that account sat unused long enough, it could tombstone (or otherwise age out of a usable state) before the workstation was moved into the clinical space, which meant the machine would fail domain authentication on install day and force rework. Imaging and final build-out therefore had to be planned against that identity lifecycle window: image early enough to protect the schedule, but defer domain join and post-join configuration until close enough to placement that the computer account would still be valid at go-live.
...
Domain computer-account aging / tombstoning was a real deployment constraint, not a footnote. Joining machines to the domain too early — long before final placement — risked accounts aging out before go-live. Planning imaging versus domain-join timing against that window prevented rework that would otherwise show up as a mysterious "workstation fell off the domain" problem on install day.
